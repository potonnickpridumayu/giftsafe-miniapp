...
// Мягкий овальный вырез под стикер на время анимации. Овал накрывает зону
// стикера (inset 20% => ±30% от центра) с плавным краем; на официальной
// картинке Fragment вокруг стикера узора и так нет — чистый градиент, поэтому
// под вырезом оказывается наш градиент тех же цветов и шва не видно.
const HOLE = 'radial-gradient(ellipse 38% 40% at 50% 46%, transparent 80%, #000 100%)'
 ...
 *
 * На время анимации картинка ОСТАЁТСЯ на экране — узор вокруг стикера 1-в-1
 * как в статике (те же пиксели, ничего не перестраивается и не «прыгает»).
 * Прячется только центральная зона со статичным стикером: мягкий овальный
 * вырез, сквозь который виден градиент тех же цветов. Чёткая векторная
 * анимация играет поверх. lottie грузится ЛЕНИВО (при первом проигрыше).
 */
