...
export default function App() {
  ...
  // (маркет/обмен/портфель), но не меньше одного полного интро (3с): если
  // данные готовы раньше — ждём конца прогона, если позже — анимация идёт по
  // кругу и мы обрываем её сразу по готовности, посреди любого кадра.
  ...
  useEffect(() => {
    ...
    // Отсчёт интро идёт от старта САМОЙ анимации (этот эффект = маунт сплэша),
    // а не от первой отрисовки страницы: до маунта успевает пройти загрузка
    // бандла (на телефоне ~секунда), и по ней первый прогон обрывался бы
    // на середине.
