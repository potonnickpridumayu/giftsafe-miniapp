...
export default function TradeDetail() {
  ...
  const handlePropose = async () => {
    ...
    try {
      await api.proposeTradeOffer(item.id, selectedGiftId, topUpNum)
      setProposed(true)
      setPicking(false)
    } catch (e) {
    ...
    <div className="page">
      ...
      {proposed ? (
        ...
        <div className="card" style={{ padding: '14px 16px' }}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10 }}>Выбери свой подарок для обмена</div>
          {myGifts === null ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 280, overflowY: 'auto', marginBottom: 12 }}>
              {myGifts.map(g => (
                <div
                  key={g.gift_id}
                  onClick={() => { haptic('light'); setSelectedGiftId(g.gift_id) }}
                  className="card"
                  style={{
                    padding: '8px 10px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer',
                    border: selectedGiftId === g.gift_id ? '1px solid var(--gold)' : '1px solid var(--border)',
                  }}
                >
                  ...
                  {selectedGiftId === g.gift_id && <span style={{ color: 'var(--gold)' }}>✓</span>}
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-primary" style={{ flex: 1 }} onClick={handlePropose} disabled={proposing || !selectedGiftId}>
              {proposing ? '⏳ Отправляем...' : 'Предложить'}
            </button>
